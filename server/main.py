...
socketio = SocketIO(app, cors_allowed_origins="*", logger=True,
                    engineio_logger=True, manage_session=False,)
...
def upload_file():
    ...
    if (index > 1):
        ...
        stream = ffmpeg.output(stream, f'./recs/wav/{clipname}_{username}.wav')
        ffmpeg.run(stream, overwrite_output=True)

        score = calculateScore(
            f'./recs/wav/{clipname}_{username}.wav', f'./clips/{clipname}.wav')
...
def start():
    socketio.emit('gameStart',None, broadcase=True)
